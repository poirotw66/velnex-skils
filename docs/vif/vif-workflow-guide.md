...
**Findings 處理**：每個 finding 依嚴重度分級（🔴🟠🟡🟢）。🔴🟠 必須修復，🟡🟢 由 Human 決定修復或接受。
...
| Audit | ✅ | 2 moderate (pre-existing, accepted) |
...
| 等級 | 含義 | 處理方式 |
|------|------|---------|
| 🔴 Critical | 正確性、安全性、規格必要功能未實作 | 必須修復 |
| 🟠 High | 規格行為部分不符、重大功能缺口 | 必須修復 |
| 🟡 Medium | 影響維護性/效能、命名不一致 | AI 建議 → Human 決定 |
| 🟢 Low | 規格外的改善建議（風格、偏好） | AI 建議 → Human 決定 |
...
| Phase 4: Review | ✔️ APPROVED | 0 🔴、0 🟠、2 🟡（已修復）、2 🟢（接受） |
