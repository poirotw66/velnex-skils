...
### Claude Code（完整體驗：skills + agents + marketplace）

```bash
# 加入 marketplace
/plugin marketplace add /path/to/velnex-skils
# 或 GitHub
/plugin marketplace add poirotw66/velnex-skils

# 安裝 plugin（需一併安裝 vex 以取得 git-commit agent）
...
派遣規則：`plugins/vif/references/cursor-subagent-dispatch.md`、`codex-subagent-dispatch.md`
