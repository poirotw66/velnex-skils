...
讀取 CLAUDE.md `AI Cross-Review` 設定，`verify` 已啟用時，在 security-reviewer 完成後觸發。傳入本次變更的 source code 檔案。

執行：呼叫設定的 AI CLI 進行獨立安全審查 → 比對 security-reviewer 結果 → 新發現加入 Verification Report。
