...
| 步驟 | 正常流程 | God Mode |
|------|---------|----------|
| Optional Stage 詢問 | CLAUDE.md 未設定時問 Human | 依 CLAUDE.md 設定決定，未設定則跳過，不問 |
| 🟡🟢 findings | Human 選修或跳 | AI 直接修復（做最佳判斷）→ 重跑完整 verify |
| 修復循環上限 | max 3 次 | 同（max 3 次，仍有 🔴🟠 → 中止 God Mode） |

> God Mode 的 🟡🟢 處理比正常流程嚴格：正常流程由 Human 決定修或跳，God Mode 全部修復。所有修復記錄到 Results Report，供使用者最終確認。
