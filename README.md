...
/vif-spec ───────────────────────────── AI 直接
    │  影響分析 → 撰寫 spec.md
    │  Step 4 Review ──▶ 派遣 spec-reviewer
    │  選擇器 → 展開設計文件？
    │ Human approve → commit
    ▼
...
/vif-verify ─────────────────────────── Core stages：
    │  Stage 1-5 ──▶ 派遣 verifier
    │  Stage 6 ────▶ 派遣 security-reviewer
    │  Optional:
    │    Stage 7 ──▶ /simplify（Claude Code）
    │    Stage 8 ──▶ 派遣 verifier（Design Doc）
    ▼
...
```
PD/PM                Designer            SA/SD             Frontend     Backend
  │                     │                  │                  │            │
  ▼                     ▼                  ▼                  ▼            ▼
/vif-prd          Figma/Prototype      /vif-spec          /vif-ui-spec  /vif-api-spec
/vif-bdd(可選)          │            (影響分析+規劃)      (頁面規格)    (API+openapi
  │                     │                  │                  │         +dbschema)
  │                     │                  │                  │            │
  └─────────────────────┴────────┬─────────┘                  │            │
                                 │                            │            │
...
| **SA/SD** | `/vif-spec` | PRD + Figma | `docs/specs/NNN/spec.md` |
| **Frontend** | `/vif-ui-spec` | Figma + Spec | `docs/ui-specs/**/*.md` |
