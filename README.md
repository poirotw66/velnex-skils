...
```bash
npx github:poirotw66/velnex-skils install -g --cursor-only
...
```bash
npx skills add poirotw66/velnex-skils -a cursor -a codex -g -y
```

只裝 subagents：

```bash
npx github:poirotw66/velnex-skils agents -g
```
...
# 全域 subagents
npx github:poirotw66/velnex-skils uninstall -g
...
```
velnex/
├── package.json                       ← npx 安裝入口（velnex-skils）
├── bin/velnex-cursor.mjs              ← npx 執行檔
├── scripts/
│   └── cursor/                        ← Cursor 安裝腳本（skills + subagents）
├── .claude-plugin/
